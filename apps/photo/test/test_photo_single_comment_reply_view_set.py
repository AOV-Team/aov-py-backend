...
class TestPhotoSingleCommentReplyViewSetPOST(TestCase):
    def setUp(self):
        ...
        FCMDevice.objects.create(registration_id=str(uuid.uuid4()).replace("-", ""), user=commenter, type="ios")
    ...
    def test_photo_single_comment_reply_view_set_post_successful(self):
        ...
        original_commenter = User.objects.get(username="testuser")
        photo_owner = User.objects.get(username="aov1")
        photo = photo_models.Photo.objects.get(user=photo_owner)
        photo_comment = photo_models.PhotoComment.objects.get(user=original_commenter)
        original_commenter_device = FCMDevice.objects.get(user=original_commenter)
        ...
        with mock.patch('fcm_django.fcm.fcm_send_bulk_message') as p:
            ...
            p.assert_called_with(api_key=None, badge=None, data=None, icon=None, sound=None, title=None,
                body="{} replied to your comment, {}.".format(photo_owner.username, original_commenter.username),
                registration_ids=[original_commenter_device.registration_id])
    ...
    def test_photo_single_comment_view_set_post_with_tagged_users(self):
        ...
        with mock.patch('fcm_django.fcm.fcm_send_bulk_message') as p:
            ...
            calls.append(mock.call(api_key=None, badge=None, data=None, icon=None, sound=None, title=None,
                                   body="{} replied to your comment, {}.".format(photo_owner.username, user.username),
                                   registration_ids=[device.registration_id]))
            calls.append(mock.call(api_key=None, badge=None, data=None, icon=None, sound=None, title=None,
                                   body="{} mentioned you in a comment, {}.".format(photo_owner.username,
                                                                                    tagged_one.username),
