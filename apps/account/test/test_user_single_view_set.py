...
class TestUserSingleViewSet(TestCase):
    ...
    def test_single_user_view_set_filter_username_successful(self):
        """
        Test that we can get a user by their username
        ...
        request = client.get('/api/users/{}?username={}'.format(user.id, user.username), format='json')
        result = request.data
